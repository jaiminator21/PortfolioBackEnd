...
const postGame = async (req, res) => {
  try {
    ...
    /* if (req.file) {
      newGame.logo = req.file.path; // If there is a file attached to the request, set its path as the logo for the new game
    } */
...
const deleteGame = async (req, res) => {
  try {
    ...
    if (deleteGame.logo.includes("cloudinary")) {
      deleteFile(deleteGame.logo); // If the game's logo is stored in Cloudinary, delete it using deleteFile function
    }
